...
| 中转站 `gpt-5.5`、`gpt-5.5-medium` | HTTP 404：该密钥所在分组没有配置这个模型 |
| 中转站 `grok-4.6` | HTTP 502（32 秒） |
| 中转站 `grok-4.5` | HTTP 200 但编造内容：测试图被说成“可控系统动力学”段落，一张 AIUI Studio 截图被说成“USDC 支付页面”；没有图片时它会正确回答 `NO_IMAGE` |

要让识别真正可用，需要一个能读图的模型：在中转站后台给这个密钥所在的分组开通 `gpt-5.5`（或换一个支持图片输入的模型），再跑一次上面的验证。
...
- 能读图的模型从头到尾跑通（当前密钥没有 `gpt-5.5`，可用的两个 grok 模型不读图）。
- 中转站在真机上的连通性、延迟和图片大小上限。
